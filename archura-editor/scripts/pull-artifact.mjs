...
if (!siteMatch && !designMatch) {
  console.error('Usage: pull-artifact.mjs sites/<site> | orgs/<orgId>/designs/<designId> [--as <name>] [--drafts]');
  process.exit(1);
}
...
if (objects.length === 0) {
  console.error(`Nothing under ${prefix}/ — check the name and bucket.`);
  process.exit(1);
}
...
console.log(`open: ${editorUrl}  (dev server, e.g. npm run dev)`);
