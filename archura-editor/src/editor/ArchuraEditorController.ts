...
export class ArchuraEditorController {
  ...
  #injectCanvasHints(editor: GrapesEditor): void {
    ...
    const hint = 'cursor: text; outline: 1px dashed rgba(99, 102, 241, 0.45); outline-offset: 2px;';
    const rules: string[] = [];
    for (const [tag, parts] of this.#stylePartsByTag) {
      for (const part of Object.keys(parts)) {
        if (part === 'host') continue;
        // Emit both selectors; each only matches its own mechanism (shadow
        // components use part=, light-DOM components use data-part=).
        rules.push(`${tag}::part(${part}):hover { ${hint} }`);
        rules.push(`${tag} [data-part="${part}"]:hover { ${hint} }`);
      }
    }
  ...
  #partSelector(component: Component, part: string, shadow: boolean): string {
    // Shadow components expose parts via ::part(); light-DOM components (which
    // Stripe Elements require) mark elements with data-part= and are targeted
    // by an id-scoped descendant selector. Both round-trip through the artifact.
    return shadow
      ? `#${component.getId()}::part(${part})`
      : `#${component.getId()} [data-part="${part}"]`;
  }

  #activatePart(component: Component, part: string, shadow: boolean): void {
    const editor = this.#gjsEditor;
    if (!editor) return;
    // The rule needs the element id in the exported html
    component.addAttributes({ id: component.getId() });
    this.#activePart = { component, part };
    // Create the rule explicitly: selecting by string can lose the pseudo/
    // descendant structure when the selector round-trips through the parser
    const selector = this.#partSelector(component, part, shadow);
  ...
  #handleCanvasClick(editor: GrapesEditor, event: Event, selectedAtMousedown: Component | null): void {
    ...
    const attr = (node: unknown, name: string) =>
      typeof (node as Element)?.getAttribute === 'function' ? (node as Element).getAttribute(name) : null;

    const partEl = path.find((node) => attr(node, 'part') || attr(node, 'data-part')) as Element | undefined;
    if (!partEl) {
      if (this.#activePart) this.clearActivePart();
      return;
    }
    const shadow = !!partEl.getAttribute('part');
    const part = (partEl.getAttribute('part') ?? partEl.getAttribute('data-part'))!;
    // The host component is the nearest node in the composed path whose tag is
    // a registered component (works for both shadow host and light-DOM ancestor)
    const host = path.find((node) =>
      this.#stylePartsByTag.has(String((node as Element)?.tagName ?? '').toLowerCase())
    ) as Element | undefined;
    const declared = host && this.#stylePartsByTag.get(host.tagName.toLowerCase());
    if (!host || !declared?.[part]) {
      if (this.#activePart) this.clearActivePart();
      return;
    }
    ...
    setTimeout(() => this.#activatePart(component, part, shadow), 0);
  }
