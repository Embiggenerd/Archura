...
export function buildComponentSourcePath(componentPath: string[]) {
  return `${COMPONENT_SOURCE_ROOT}/${componentPath.join('/')}.js`;
}

export function buildComponentModuleUrl(componentPath: string[]) {
  return `/@fs/${buildComponentSourcePath(componentPath)}`;
}
...
export async function loadComponent(componentPath: string[]): Promise<LoadedComponent> {
  ...
  const sourcePath = buildComponentSourcePath(componentPath);
  const moduleUrl = buildComponentModuleUrl(componentPath);
