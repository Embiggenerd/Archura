...
type repository interface {
	...
	OrganizationByID(context.Context, string) (store.Organization, error)
...
func (s *Server) Router() http.Handler {
	...
	r.Route("/v1/admin", func(r chi.Router) {
		...
		// Reachable with the owner role alone — the console needs context to know
		// whether to prompt, and step-up itself can't require step-up.
		r.Get("/context", s.handleAdminContext)
		r.Post("/mfa/enroll", s.handleAdminMFAEnroll)
		r.Post("/mfa/activate", s.handleAdminMFAActivate)
		r.Post("/mfa/verify", s.handleAdminMFAVerify)
		// Sensitive operations additionally require step-up MFA in prod.
		r.Group(func(r chi.Router) {
			r.Use(s.requireAdminElevation)
