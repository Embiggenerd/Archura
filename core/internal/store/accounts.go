...
func (s *Store) AccountByEmail(ctx context.Context, email string) (Account, error) {
	var account Account
	err := s.Pool.QueryRow(ctx, `
		SELECT id::text, email, email_verified_at, COALESCE(staff_role, ''), created_at
		FROM accounts
		WHERE email = $1`, email,
...
func upsertAccount(ctx context.Context, tx pgx.Tx, email string) (Account, bool, error) {
	...
	err := tx.QueryRow(ctx, `
		...
		RETURNING id::text, email, email_verified_at, COALESCE(staff_role, ''), created_at`, email,
	).Scan(&account.ID, &account.Email, &account.EmailVerifiedAt, &account.StaffRole, &account.CreatedAt)
	if err == nil {
...
func (s *Store) AccountByID(ctx context.Context, id string) (Account, error) {
	var account Account
	err := s.Pool.QueryRow(ctx, `SELECT id::text, email, email_verified_at, COALESCE(staff_role, ''), created_at FROM accounts WHERE id = $1::uuid`, id).
		Scan(&account.ID, &account.Email, &account.EmailVerifiedAt, &account.StaffRole, &account.CreatedAt)
	if err != nil {
