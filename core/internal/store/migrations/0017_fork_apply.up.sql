-- Fork apply (docs/PLAN_FILE_ON_DEMAND.md step 3): a ready fork's published
-- set can be applied over its source design. The row records the terminal
-- 'applied' state; the apply/rejection details live in the audit log.
ALTER TABLE designs
    DROP CONSTRAINT designs_fork_status_check;

ALTER TABLE designs
    ADD CONSTRAINT designs_fork_status_check
        CHECK (fork_status IN ('pending', 'ready', 'failed', 'applied'));
