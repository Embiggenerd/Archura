...
follow-ups (site manifest, page switcher, Worker path routing); collections and the
articles app are specced in `ARTICLES_APP.md` (Markdown-canonical core records, math +
wikilink graph, draft/published, list/detail components, locked template + free document, AEO). This file tracks
...
**Light-DOM components too (verified by `scripts/verify-stripe.mjs`).** Components that
must render in light DOM (Stripe Elements can't mount in shadow roots) mark parts with
`data-part="…"` instead of `part="…"`; selection targets an id-scoped descendant rule
`#id [data-part="name"]` instead of `#id::part(name)`. The click hit-test recognizes both
attributes and finds the host as the nearest registered-component tag in the composed
path; hover hints emit both selector variants (each only matches its own mechanism). The
descendant rule round-trips through the artifact and re-applies when Lit renders the
labels on the deployed page. This unblocks per-part styling for all future light-DOM /
app-scale components (e.g. the articles app). First case: recoloring just the Stripe
component's "Card number" label.
